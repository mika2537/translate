...
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { Download, RotateCcw, CheckCircle, Settings } from 'lucide-react';
import { useTranslation } from './hooks/useTranslation';
...
const VideoResult = ({ videoUrl = "/videos/ted_video_en.mp4", onNewTranslation }: VideoResultProps) => {
  const { t } = useTranslation();
  ...
  const switchSubtitle = (lang: 'en' | 'mn' | 'off') => {
    ...
    for (let i = 0; i < video.textTracks.length; i++) {
      ...
      track.mode = lang === 'off' ? 'disabled' : (track.language === lang ? 'showing' : 'disabled');
    }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="text-center space-y-2">
        <h2 className="text-3xl font-bold text-foreground">{t('translate.completed')}</h2>
        <p className="text-muted-foreground">{t('translate.completedDescription')}</p>
      </div>
        ...
        <CardContent className="p-0">
          <div className="relative aspect-video bg-black flex items-center justify-center">
            ...
            <div className="absolute top-2 right-2 z-10">
              ...
              {showMenu && (
                <div className="mt-2 absolute right-0 bg-black/80 text-white text-sm rounded-md shadow-lg">
                  <button
          ...
          <div className="p-6 space-y-4">
            <div className="flex flex-col sm:flex-row gap-3">
              <Button variant="default" size="lg" className="flex-1" onClick={downloadVideo}>
                <Download className="w-5 h-5" />
                {t('translate.download')}
              </Button>
